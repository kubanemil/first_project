...
def welcome(message):
    ...
    bot.send_message(message.chat.id, "Комманды для бота: /estimate, /send_photo, /send_music, /send_sticker, /game")
    bot.send_message(message.chat.id, "Также можно сыграть в игру 'Jojo's Theme': /game")
...
def reply(message):
    if message.text == names[0]:
        bot.send_voice(message.chat.id, 'AwACAgQAAxkBAAIJ-WDHBbB5ASKrQap5PNb0r6e3q0gtAAI9OAACOYo4Ujp3blarpYLcHwQ')
    else:
        bot.send_voice(message.chat.id, 'AwACAgQAAxkBAAIJ8WDHBOCgXtUr4QwknXXmvUxp2qCtAAIOOAACOYo4UpIvS_-LR_RxHwQ')
    markup = types.ReplyKeyboardRemove()
    bot.send_message(message.chat.id, "End", reply_markup=markup)
